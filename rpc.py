...
Shared plumbing for the one hop pipeline. Not run directly.

Everything that talks to the node goes through here so the awkward parts
are solved once: the gateway's block range cap is read out of its own
refusal rather than found by halving, request bodies are gzipped if the
gateway accepts them, every sweep is resumable at a 1,000 block grid,
and a failed write can never mark its blocks as finished.
"""
...
OVERFLOW_HINTS = ("too many", "limit", "exceed", "too large", "out of memory",
                  "deadline", "timeout", "canceled", "cancelled", "response size")
...
class Node:
    ...
    def _send(self, payload, timeout):
        if _use_gzip:
            ...
            return self.session.post(
                self.url, data=buffer.getvalue(), timeout=timeout,
    ...
    def call(self, method, params, timeout=None, stop=None):
        answer = self._post({"jsonrpc": "2.0", "id": 0, "method": method,
                             "params": params}, timeout, stop)
        if "error" in answer:
            raise classify(answer["error"])
        return answer["result"]

    def batch(self, calls, timeout=None, stop=None):
        """calls is [(method, params), ...]. Returns results in order."""
        if not calls:
            return []
...
def probe_gzip(node, probe_params):
    """Compressed bodies cut a large address filter roughly fourfold."""
    global _use_gzip
    _use_gzip = True
    try:
...
def discover_cap(node, method, build):
    """
    Widest block range the gateway accepts, probed with a cheap filter.

    build(low, high) returns the params for a probe. Probing with the real
    filter would upload megabytes per rejection, so callers pass a small one.
    """
    width = WINDOW_MAX
    while width > WINDOW_MIN:
        try:
            node.call(method, build(1_000_000, 1_000_000 + width - 1),
                      timeout=180)
            note_limit(width)
            return width
...
def open_db(path, schema):
    ...
    con.execute("CREATE TABLE IF NOT EXISTS hot_keys "
...
class RangeSweeper:
    ...
    build(low, high)  -> (method, params) for one window
    rows(result)      -> list of tuples matching insert_sql
    hub_index         -> position in the tuple to guard on, or None
    """

    def __init__(self, db_path, schema, insert_sql, build, rows,
                 hub_index=None, hub_limit=0, workers=WORKERS, label="rows"):
    ...
    def _guard(self, rows):
        ...
        if not self.hot:
            return rows
        return [r for r in rows if r[self.hub_index] not in self.hot]
        ...
        while True:
            ...
            try:
                if kind == "span":
                    ...
                    con.execute("INSERT OR REPLACE INTO hot_keys VALUES (?,?)",
                                payload)
                    con.commit()
                    pending = 0
    ...
    def _worker(self, done):
        ...
        while not self.stop.is_set():
            ...
            while current <= high and not self.stop.is_set():
                ...
                if all(u in done for u in range(current, stop_at + 1, UNIT)):
                    with self.lock:
                ...
                try:
                    ...
                    if window > WINDOW_MIN:
                        ...
                        clean = 0
                        continue
                    self.results.put(("failed", (current, stop_at,
                                                 str(problem))))
                    with self.lock:
                        self.counters["failed"] += 1
                    current = stop_at + 1
    ...
    def run(self, start, end):
        ...
        while cursor <= end:
            ...
            if not all(u in done for u in range(cursor, stop_at + 1, UNIT)):
        ...
        for _ in range(self.workers):
            thread = threading.Thread(target=self._worker, args=(done,),
                                      daemon=True)
            thread.start()
        ...
        try:
            while any(t.is_alive() for t in threads):
                ...
                print("  {:,}/{:,} blocks  {:.2f}%  {} {:,}  failed {}  "
                      "{:,.0f} blk/s  eta {}    ".format(
                          blocks, span, blocks / span * 100 if span else 100,
                          self.label, rows, failed, rate,
